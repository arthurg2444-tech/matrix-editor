...
st.set_page_config(page_title="Matrix Editor V6", layout="wide")
st.title("🎬 Matrix Editor - Versão Estável 100%")

# 1. Modelo 'small' para maior precisão (IA mais inteligente)
@st.cache_resource
def load_whisper():
...
def criar_frame_legenda(texto, largura_video, altura_video):
    ...
    # Fonte pequena e discreta (2.8% da altura)
    ...
    for linha in reversed(linhas):
        # CORREÇÃO DEFINITIVA DO ERRO DE TUPLE:
        bbox = draw.textbbox((0, 0), linha, font=font)
        w = bbox[2] - bbox[0] # Largura correta
        h = bbox[3] - bbox[1] # Altura correta
        x = (largura_video - w) / 2
        
        # Contorno Preto Fino para legibilidade
...
if video_file:
    ...
    if st.button("🚀 Gerar Vídeo Final"):
        with st.spinner("Sincronizando 2 palavras por vez..."):
            try:
                ...
                resultado = modelo.transcribe(
                    ...
                    initial_prompt="Português brasileiro, fala natural, legendas curtas."
                )
                ...
                for seg in resultado['segments']:
                    ...
                    # Agrupa de 2 em 2 palavras para ficar dinâmico
