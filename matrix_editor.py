...
st.set_page_config(page_title="Matrix Editor V4", layout="wide")
st.title("🎬 Matrix Editor - Precisão Aumentada")

# 1. USANDO MODELO 'SMALL' PARA ERRAR MENOS AS PALAVRAS
@st.cache_resource
def load_whisper():
    return whisper.load_model("small") # Troquei 'base' por 'small' (Mais preciso)
...
def criar_frame_legenda(texto, largura_video, altura_video):
    ...
    # FONTE AINDA MENOR (2.8% da altura do vídeo)
    ...
    # Quebra de linha curta e limpa
    linhas = textwrap.wrap(texto.upper(), width=20)
    
    # POSIÇÃO NO RODAPÉ (88% para baixo - Mais discreto)
...
if video_file:
    ...
    if st.button("🚀 Gerar Vídeo Final"):
        with st.spinner("IA 'Small' processando com alta precisão..."):
            try:
                # Transcrição com reforço de dicionário
                resultado = modelo.transcribe(
                    temp_path, 
                    language="pt", 
                    word_timestamps=True,
                    initial_prompt="Este vídeo contém termos comuns do dia a dia em português brasileiro."
                )
                ...
                for seg in resultado['segments']:
                    ...
                    # Agrupa de 2 em 2 palavras para ser ainda mais rápido e preciso
                    for i in range(0, len(palavras), 2):
                        ...
                        fim_t = seg['start'] + ((i+2) / len(palavras)) * duracao_seg
                        
                        if fim_t > seg['end']: fim_t = seg['end']

                        img_legenda = criar_frame_legenda(trecho, clip_original.w, clip_original.h)
                        
                        txt_clip = (ImageClip(img_legenda)
                                    .set_start(inicio_t)
                                    .set_duration(fim_t - inicio_t)
